#!/usr/bin/env python3
"""
Generador completo del catálogo LifePlus con TODOS los productos (118)
"""
...
def generate_product_card(product):
    """Genera HTML para una tarjeta de producto"""
    code = product["code"]
    name = product["name"]
    price = product["price"]
    ...
    badge = ""
    if "VEGAN" in name:
        badge = '<div class="badge">🌱 Vegano</div>'
    elif "Purple Flash" in name:
        badge = '<div class="badge">⭐ Más Vendido</div>'
    elif "Gold" in name and "Maintain" in name:
        badge = '<div class="badge">⭐ Premium</div>'
...
def main():
    ...
    for product in products_data:
        all_cards += generate_product_card(product)

    print(f"✅ Generadas {len(products_data)} tarjetas de productos")
    print("📁 Tarjetas HTML listas para integrar")

    # Guardar las tarjetas en un archivo para referencia
    with open('all_product_cards.html', 'w', encoding='utf-8') as f:
        f.write(all_cards)
